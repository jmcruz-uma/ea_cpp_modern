// Benchmark: ea-cpp vs jMetal — same generations, same parameters
// 250 generations × 100 pop_size = 25000 evaluations
// Compile: g++-14 -std=c++23 -O2 -I include benchmark_fair.cpp -o benchmark_fair -lm
...
int main() {
    // Same parameters as jMetal benchmark
    const int POP_SIZE = 100;
    const int MAX_EVALS = 25000;  // = 250 generations × 100 pop
    const int DIM = 30;
    const int N_OBJ = 2;
    const int RUNS = 10;

    std::cout << "=== ea-cpp Benchmark: NSGA-II on ZDT1 ===" << std::endl;
    std::cout << "Population: " << POP_SIZE << std::endl;
    std::cout << "Max evaluations: " << MAX_EVALS << std::endl;
    std::cout << "Dimensions: " << DIM << std::endl;
    std::cout << "Runs: " << RUNS << std::endl;
    std::cout << "Crossover probability: 0.9" << std::endl;
    std::cout << "Crossover distribution index: 20.0" << std::endl;
    std::cout << "Mutation probability: 1.0/30" << std::endl;
    std::cout << "Mutation distribution index: 20.0" << std::endl;
    std::cout << std::endl;
    ...
    for (int r = 0; r < RUNS; ++r) {
        ...
        // Same parameters as jMetal
        double crossoverProbability = 0.9;
        double crossoverDistributionIndex = 20.0;
        double mutationProbability = 1.0 / DIM;
        double mutationDistributionIndex = 20.0;

        ea::NSGAII<ea::SBXCrossover, ea::PolynomialMutation> nsga;
        nsga.crossover.distribution_index = crossoverDistributionIndex;
        nsga.crossover.crossover_probability = crossoverProbability;
        nsga.mutation.distribution_index = mutationDistributionIndex;
        // Note: ea-cpp polynomial mutation uses distribution_index, not mutation_probability separately
        // In jMetal, mutation probability = 1/dim by default
