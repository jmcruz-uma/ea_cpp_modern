/// @file paes_runner.cpp
/// @brief PAES benchmark runner — ea_cpp_modern vs jMetal.
///
/// Corre PAES sobre ZDT1-ZDT4, NUM_RUNS veces por problema.
/// Salida CSV por stdout: system,problem,run,time_ms,igd
///
/// Compilar (benchmark):
///   g++-14 -std=c++23 -O3 -march=native -DNDEBUG -I include examples/paes_runner.cpp -o build/paes_runner -lm
///
/// Uso:
///   ./build/paes_runner [NUM_RUNS [BASE_SEED]]
...
// ── Parámetros (deben coincidir con JMetalPAESBenchmark.java) ────────────────
static constexpr int      MAX_EVALS    = 25000;
static constexpr int      ARCHIVE_SIZE = 100;
static constexpr int      BISECTIONS   = 5;
...
// ── Un run de PAES — devuelve {time_ms, igd} ─────────────────────────────────
...
std::pair<double, double> run_once(Problem& prob, const std::string& prob_name,
    ...
    algo.mutation.mutation_rate       = 1.0 / dim;  // jMetal: 1/numberOfVariables
    algo.max_evals    = MAX_EVALS;
    algo.archive_size = ARCHIVE_SIZE;
    algo.bisections   = BISECTIONS;
    ...
    // PAES::run() ya rellena pop con los miembros del archivo (no dominados)
    // IGD sobre el archivo completo (igual que jMetal: algorithm.result() = archive)
    std::vector<int> all_indices(pop.pop_size);
    std::iota(all_indices.begin(), all_indices.end(), 0);

    auto ref     = make_ref_front(prob_name);
    double igd_val = ea::igd(pop, all_indices, ref, 1.0);
...
void benchmark_problem(const std::string& name, int dim, int num_runs, uint64_t base_seed) {
    ...
    for (int r = 0; r < num_runs; ++r) {
        auto [t, igd] = run_once(prob, name, base_seed + static_cast<uint64_t>(r), dim);
...
int main(int argc, char* argv[]) {
    ...
    std::cerr << "=== ea-cpp PAES Benchmark ===\n";
    std::cerr << "max_evals=" << MAX_EVALS
