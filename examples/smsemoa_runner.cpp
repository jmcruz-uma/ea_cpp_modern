/// @file smsemoa_runner.cpp
/// @brief SMS-EMOA benchmark runner — ea_cpp_modern vs jMetal.
///
/// Corre SMS-EMOA sobre ZDT1-ZDT4, NUM_RUNS veces por problema.
/// Salida CSV por stdout: system,problem,run,time_ms,igd
///
/// Compilar (benchmark):
///   g++-14 -std=c++23 -O3 -march=native -DNDEBUG -I include examples/smsemoa_runner.cpp -o build/smsemoa_runner -lm
///
/// Uso:
///   ./build/smsemoa_runner [NUM_RUNS [BASE_SEED]]
...
// ── Parámetros del benchmark (deben coincidir con JMetalSMSEMOABenchmark.java) ──
...
// ── Un run de SMS-EMOA — devuelve {time_ms, igd} ─────────────────────────────
...
std::pair<double, double> run_once(Problem& prob, const std::string& prob_name, uint64_t seed) {
    ...
    ea::SMSEMOA<ea::SBXCrossover, ea::PolynomialMutation> algo;
    ...
    // mutation_rate = -1 → auto 1/dim (igual que jMetal pm = 1/D)
    ...
    // IGD sobre el frente no dominado (mismo criterio que jMetal: algorithm.result())
...
int main(int argc, char* argv[]) {
    ...
    std::cerr << "=== ea-cpp SMS-EMOA Benchmark ===\n";
    std::cerr << "pop=" << POP_SIZE << "  max_evals=" << MAX_EVALS
              << "  offset=100.0  runs=" << num_runs << "  seed=" << base_seed << "\n";
