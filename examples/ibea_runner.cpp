/// @file ibea_runner.cpp
/// @brief IBEA MO benchmark runner — ea_cpp_modern vs jMetal.
///
/// Corre IBEA sobre ZDT1-ZDT4, NUM_RUNS veces por problema.
/// Salida CSV por stdout: system,problem,run,time_ms,igd
///
/// Compilar (benchmark):
///   g++-14 -std=c++23 -O3 -march=native -DNDEBUG -I include examples/ibea_runner.cpp -o build/ibea_runner -lm
///
/// Uso:
///   ./build/ibea_runner [NUM_RUNS [BASE_SEED]]
...
// ── Parámetros del benchmark (deben coincidir con JMetalIBEABenchmark.java) ──
...
// ── Un run de IBEA — devuelve {time_ms, igd} ──────────────────────────────────
...
std::pair<double, double> run_once(Problem& prob, const std::string& prob_name, uint64_t seed) {
    ...
    ea::IBEA<ea::SBXCrossover, ea::PolynomialMutation> ibea;
    ibea.pop_size                        = POP_SIZE;
    ibea.max_evals                       = MAX_EVALS;
    ibea.crossover.crossover_probability = CX_PROB;
    ibea.crossover.distribution_index   = CX_ETA;
    ibea.mutation.distribution_index    = MUT_ETA;
    // mutation_rate = -1 → auto 1/dim (matches jMetal PolynomialMutation(1/D, 20))

    auto t0 = std::chrono::high_resolution_clock::now();
    ibea.run(pop, [&](ea::Population<>& p, int idx) { prob.evaluate(p, idx); });
...
int main(int argc, char* argv[]) {
    ...
    std::cerr << "=== ea-cpp IBEA MO Benchmark ===\n";
