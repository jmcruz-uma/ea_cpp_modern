/// @file mocell_runner.cpp
/// @brief MOCell benchmark runner — ea_cpp_modern vs jMetal.
///
/// Corre MOCell sobre ZDT1-ZDT4, NUM_RUNS veces por problema.
/// Salida CSV por stdout: system,problem,run,time_ms,igd
///
/// Compilar (benchmark):
///   g++-14 -std=c++23 -O3 -march=native -DNDEBUG -I include examples/mocell_runner.cpp -o build/mocell_runner -lm
///
/// Uso:
///   ./build/mocell_runner [NUM_RUNS [BASE_SEED]]
...
// ── Parámetros (deben coincidir con JMetalMOCellBenchmark.java) ──────────────
...
// ── Frente de referencia analítico ────────────────────────────────────────────
...
// ── Un run de MOCell — devuelve {time_ms, igd} ───────────────────────────────
template <typename Problem>
std::pair<double, double> run_once(Problem& prob, const std::string& prob_name,
                                   uint64_t seed, double mut_prob) {
    ...
    algo.crossover.crossover_probability = CX_PROB;
    algo.crossover.distribution_index    = CX_ETA;
    algo.mutation.mutation_rate          = mut_prob;
    algo.mutation.distribution_index     = MUT_ETA;
    algo.pop_size    = POP_SIZE;
    algo.max_evals   = MAX_EVALS;
    algo.archive_size = POP_SIZE;
    algo.grid_rows   = 10;
    algo.grid_cols   = 10;
    ...
    // IGD sobre el archivo (igual que jMetal: algorithm.result() = archive)
    auto ref = make_ref_front(prob_name);
    // pop.pop_size = arc_count after run
    std::vector<int> all_idx(pop.pop_size);
    std::iota(all_idx.begin(), all_idx.end(), 0);
    double igd_val = ea::igd(pop, all_idx, ref, 1.0);
...
void benchmark_problem(const std::string& name, int dim, int num_runs, uint64_t base_seed) {
    ...
    Problem prob(dim);
    double mut_prob = 1.0 / static_cast<double>(dim);
    for (int r = 0; r < num_runs; ++r) {
        auto [t, igd] = run_once(prob, name, base_seed + static_cast<uint64_t>(r), mut_prob);
