/// @file moead_runner.cpp
/// @brief MOEA/D-DE MO benchmark runner — ea_cpp_modern vs jMetal.
///
/// Corre MOEA/D-DE sobre ZDT1-ZDT4, NUM_RUNS veces por problema.
/// Salida CSV por stdout: system,problem,run,time_ms,igd
///
/// Compilar (benchmark):
///   g++-14 -std=c++23 -O3 -march=native -DNDEBUG -I include examples/moead_runner.cpp -o build/moead_runner -lm
///
/// Uso:
///   ./build/moead_runner [NUM_RUNS [BASE_SEED]]
...
// ── Parámetros del benchmark (deben coincidir con JMetalMOEADBenchmark.java) ──
static constexpr int      POP_SIZE      = 100;
static constexpr int      MAX_EVALS     = 25000;
static constexpr int      NEIGHBOR_SIZE = 20;        // T en Zhang & Li
static constexpr double   NEIGHBOR_PROB = 0.9;       // δ: prob de usar vecindario
static constexpr int      MAX_REPLACED  = 2;         // nr
static constexpr double   DE_CR         = 1.0;       // crossover rate
static constexpr double   DE_F          = 0.5;       // scaling factor
...
// ── Un run de MOEA/D-DE — devuelve {time_ms, igd} ────────────────────────────
...
std::pair<double, double> run_once(Problem& prob, const std::string& prob_name, uint64_t seed) {
    ...
    alg.neighbor_size         = NEIGHBOR_SIZE;
    alg.neighborhood_prob     = NEIGHBOR_PROB;
    alg.max_replaced_solutions = MAX_REPLACED;
    alg.aggregation_type      = ea::AggregationType::Tchebycheff;
    alg.crossover.cr          = DE_CR;
    alg.crossover.f           = DE_F;
    alg.crossover.variant     = ea::DEVariant::Rand1Bin;
    ...
    front.reserve(POP_SIZE);
    for (int i = 0; i < POP_SIZE; ++i)
        front.push_back(std::vector<double>(pop.objectives_ptr(i), pop.objectives_ptr(i) + n_obj));

    auto ref = make_ref_front(prob_name);
    double igd_val = ea::igd(front, ref, 1.0);
...
int main(int argc, char* argv[]) {
    ...
    std::cerr << "=== ea-cpp MOEA/D-DE MO Benchmark ===\n";
    std::cerr << "pop=" << POP_SIZE << "  max_evals=" << MAX_EVALS
              << "  T=" << NEIGHBOR_SIZE << "  delta=" << NEIGHBOR_PROB
              << "  CR=" << DE_CR << "  F=" << DE_F << "\n";
