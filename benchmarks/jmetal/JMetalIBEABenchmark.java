...
/**
 * jMetal 7.4 IBEA benchmark runner — ZDT1-ZDT4.
 *
 * Parámetros (deben coincidir con ibea_runner.cpp):
 *   Population size : 100
 *   Archive size    : 100
 *   Max evaluations : 25000  (100 init + 249×100 offspring)
 *   SBXCrossover    : pc=0.9, eta_c=20
 *   PolynomialMut   : pm=1/D, eta_m=20
 *   Variant         : IBEA (HVI indicator, not mIBEA)
 *   Selection       : BinaryTournamentSelection<FitnessComparator> (IBEABuilder default)
 *   ZDT4            : dim=10  (resto: dim=30)
 *
 * Salida CSV: system,problem,run,time_ms,igd
 *
 * Uso:
 *   javac -cp <jmetal-jars> JMetalIBEABenchmark.java
 *   java  -server -Xms512m -Xmx2g -XX:+UseG1GC -XX:+AlwaysPreTouch \
 *         -XX:+DisableExplicitGC -Djava.awt.headless=true \
 *         -cp <jmetal-jars>:. JMetalIBEABenchmark results_jmetal.csv [NUM_RUNS [BASE_SEED]]
 */
public class JMetalIBEABenchmark {

    static final int    POP_SIZE    = 100;
    static final int    ARCHIVE_SIZE = 100;
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.err.println("=== jMetal 7.4 IBEA MO Benchmark ===");
        System.err.printf(
            "pop=%d  archive=%d  max_evals=%d  cx_prob=%.1f  cx_eta=%.1f  mut_eta=%.1f  runs=%d  seed=%d%n",
            POP_SIZE, ARCHIVE_SIZE, MAX_EVALS, CX_PROB, CX_ETA, MUT_ETA, numRuns, baseSeed);
    ...
    static List<DoubleSolution> runOnce(DoubleProblem problem) {
        ...
        // IBEABuilder defaults: IBEA variant (HVI), BinaryTournamentSelection<FitnessComparator>
        IBEA<DoubleSolution> algorithm =
            new IBEABuilder(problem)
                ...
                .setCrossover(new SBXCrossover(CX_PROB, CX_ETA))
                .setMutation(new PolynomialMutation(pm, MUT_ETA))
                // variant defaults to IBEAVariant.IBEA (HVI indicator, not mIBEA)
                .build();
        algorithm.run();
        return algorithm.result();
    }

    // IGD con REF_PTS puntos (mismo criterio que ibea_runner.cpp: p=1.0)
