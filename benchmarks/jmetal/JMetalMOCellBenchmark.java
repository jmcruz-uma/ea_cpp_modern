...
/**
 * jMetal 7.4 MOCell benchmark runner — ZDT1-ZDT4.
 * Usa MOCellBuilder con SBX crossover, polynomial mutation, C9 neighborhood, CrowdingDistanceArchive.
 * result() devuelve el archivo (non-dominated solutions).
 *
 * Parámetros (deben coincidir con mocell_runner.cpp):
 *   Population       : 100
 *   Max evaluations  : 25000  (offspring only; initial pop not counted)
 *   Archive size     : 100
 *   CX prob          : 0.9
 *   CX eta           : 20.0
 *   Mut eta          : 20.0
 ...
 *
 * Uso:
 *   javac -cp <jmetal-jars> JMetalMOCellBenchmark.java
 *   java  -server -Xms512m -Xmx2g -XX:+UseG1GC -XX:+AlwaysPreTouch \
 *         -XX:+DisableExplicitGC -Djava.awt.headless=true \
 *         -cp <jmetal-jars>:. JMetalMOCellBenchmark results_jmetal.csv [NUM_RUNS [BASE_SEED]]
 */
public class JMetalMOCellBenchmark {
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.err.println("=== jMetal 7.4 MOCell Benchmark ===");
        System.err.printf("pop=%d  max_evals=%d  cx_prob=%.1f  cx_eta=%.1f  mut_eta=%.1f  runs=%d  seed=%d%n",
                          POP_SIZE, MAX_EVALS, CX_PROB, CX_ETA, MUT_ETA, numRuns, baseSeed);
    ...
    static List<DoubleSolution> runOnce(DoubleProblem problem, int dim) {
        ...
        var algorithm = new MOCellBuilder<>(problem, crossover, mutation)
            .setSelectionOperator(selection)
            .setMaxEvaluations(MAX_EVALS)
            .setPopulationSize(POP_SIZE)
            .setArchive(new CrowdingDistanceArchive<>(POP_SIZE))
            .build();
