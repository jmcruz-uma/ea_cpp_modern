...
/**
 * jMetal 7.4 SMPSO benchmark runner — ZDT1-ZDT4.
 *
 * Parámetros (deben coincidir con smpso_runner.cpp):
 *   Swarm size    : 100
 *   Max iterations: 250  (→ 25000 evals: 100 init + 249*100)
 *   Archive size  : 100
 *   Mutation      : PM,  pm=1/D, eta_m=20
 *   Inertia       : w=0.1 (constante)
 *   changeVelocity: -1 (bounce)
 *   ZDT4          : dim=10  (resto: dim=30)
 *
 * Salida CSV: system,problem,run,time_ms,igd
 *
 * Uso:
 *   javac -cp <jmetal-jars> JMetalSMPSOBenchmark.java
 *   java  -server -Xms512m -Xmx2g -cp <jmetal-jars>:. JMetalSMPSOBenchmark results_jmetal.csv [NUM_RUNS [BASE_SEED]]
 */
public class JMetalSMPSOBenchmark {

    static final int    SWARM_SIZE     = 100;
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.err.println("=== jMetal 7.4 SMPSO MO Benchmark ===");
        System.err.printf("swarm=%d  max_iter=%d  archive=%d  runs=%d  seed=%d%n",
                          SWARM_SIZE, MAX_ITERATIONS, ARCHIVE_SIZE, numRuns, baseSeed);
    ...
    static List<DoubleSolution> runOnce(DoubleProblem problem) {
        ...
        var archive = new CrowdingDistanceArchive<DoubleSolution>(ARCHIVE_SIZE);
    ...
    // IGD con REF_PTS puntos (mismo criterio que smpso_runner.cpp: p=1.0)
