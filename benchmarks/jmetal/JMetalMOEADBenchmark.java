...
/**
 * jMetal 7.4 MOEA/D-DE benchmark runner — ZDT1-ZDT4.
 *
 * Parámetros (deben coincidir con moead_runner.cpp):
 *   Population size              : 100
 *   Max evaluations              : 25000
 *   Neighborhood size (T)        : 20
 *   Neighborhood prob (delta)    : 0.9
 *   Max replaced solutions (nr)  : 2
 *   Function type                : TCHE
 *   DE crossover                 : CR=1.0, F=0.5, RAND_1_BIN
 ...
 *
 * Uso:
 *   javac -cp <jmetal-jars> JMetalMOEADBenchmark.java
 *   java  -server -Xms512m -Xmx2g -XX:+UseG1GC -XX:+AlwaysPreTouch \
 *         -XX:+DisableExplicitGC -Djava.awt.headless=true \
 *         -cp <jmetal-jars>:. JMetalMOEADBenchmark results_jmetal.csv [NUM_RUNS [BASE_SEED]]
 */
public class JMetalMOEADBenchmark {

    static final int    POP_SIZE      = 100;
    static final int    MAX_EVALS     = 25000;
    static final int    NEIGHBOR_SIZE = 20;
    static final double NEIGHBOR_PROB = 0.9;
    static final int    MAX_REPLACED  = 2;
    static final double DE_CR         = 1.0;
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.err.println("=== jMetal 7.4 MOEA/D-DE MO Benchmark ===");
        System.err.printf(
            "pop=%d  max_evals=%d  T=%d  delta=%.1f  CR=%.1f  F=%.1f  runs=%d  seed=%d%n",
            POP_SIZE, MAX_EVALS, NEIGHBOR_SIZE, NEIGHBOR_PROB, DE_CR, DE_F, numRuns, baseSeed);
    ...
    static List<DoubleSolution> runOnce(DoubleProblem problem) {
        ...
        MOEAD algorithm = new MOEAD(
            problem,
            POP_SIZE,
            POP_SIZE,           // resultPopulationSize
            MAX_EVALS,
            mutation,
            crossover,
            MOEAD.FunctionType.TCHE,
            "",                 // dataDirectory (2-obj: generado internamente)
            NEIGHBOR_PROB,
            MAX_REPLACED,
            NEIGHBOR_SIZE
        );
    ...
    // IGD con REF_PTS puntos (mismo criterio que moead_runner.cpp: p=1.0)
