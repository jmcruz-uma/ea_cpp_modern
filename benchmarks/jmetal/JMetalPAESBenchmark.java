...
/**
 * jMetal 7.4 PAES benchmark runner — ZDT1-ZDT4.
 * Usa PAES con GenericBoundedArchive + GridDensityEstimator (comportamiento canónico).
 * result() devuelve el archivo (non-dominated solutions).
 *
 * Parámetros (deben coincidir con paes_runner.cpp):
 *   Max evaluations  : 25000
 *   Archive size     : 100
 *   Bisections       : 5
 *   Mut eta          : 20.0  (PolynomialMutation, prob=1/dim)
 ...
 *
 * Uso:
 *   javac -cp <jmetal-jars> JMetalPAESBenchmark.java
 *   java  -server -Xms512m -Xmx2g -XX:+UseG1GC -XX:+AlwaysPreTouch \
 *         -XX:+DisableExplicitGC -Djava.awt.headless=true \
 *         -cp <jmetal-jars>:. JMetalPAESBenchmark results_jmetal.csv [NUM_RUNS [BASE_SEED]]
 */
public class JMetalPAESBenchmark {

    static final int    MAX_EVALS    = 25000;
    static final int    ARCHIVE_SIZE = 100;
    static final int    BISECTIONS   = 5;
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.err.println("=== jMetal 7.4 PAES Benchmark ===");
        System.err.printf("max_evals=%d  archive=%d  bisections=%d  mut_eta=%.1f  runs=%d  seed=%d%n",
                          MAX_EVALS, ARCHIVE_SIZE, BISECTIONS, MUT_ETA, numRuns, baseSeed);
    ...
    static List<DoubleSolution> runOnce(DoubleProblem problem, int dim) {
        ...
        // PAES canonical: GenericBoundedArchive + GridDensityEstimator
        var algorithm = new PAES<DoubleSolution>(problem, MAX_EVALS, ARCHIVE_SIZE, BISECTIONS, mutation);
        algorithm.run();
        return algorithm.result();  // returns archive (non-dominated solutions)
    }
