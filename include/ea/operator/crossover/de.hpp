...
enum class DECrossoverType : uint8_t {
    Bin,
    Exp
};

/// Mutation type for DE
enum class DEMutationType : uint8_t {
    Rand,
    Best,
    RandToBest,
    CurrentToRand
...
constexpr DEVariant de_variant_from_string(std::string_view s) {
    if (s == "RAND_1_BIN") return DEVariant::Rand1Bin;
    if (s == "RAND_1_EXP") return DEVariant::Rand1Exp;
    if (s == "RAND_2_BIN") return DEVariant::Rand2Bin;
    if (s == "RAND_2_EXP") return DEVariant::Rand2Exp;
    if (s == "BEST_1_BIN") return DEVariant::Best1Bin;
    if (s == "BEST_1_EXP") return DEVariant::Best1Exp;
    if (s == "BEST_2_BIN") return DEVariant::Best2Bin;
    if (s == "BEST_2_EXP") return DEVariant::Best2Exp;
    if (s == "RAND_TO_BEST_1_BIN") return DEVariant::RandToBest1Bin;
    if (s == "RAND_TO_BEST_1_EXP") return DEVariant::RandToBest1Exp;
    if (s == "CURRENT_TO_RAND_1_BIN") return DEVariant::CurrentToRand1Bin;
    if (s == "CURRENT_TO_RAND_1_EXP") return DEVariant::CurrentToRand1Exp;
...
struct DECrossover {
    ...
    static constexpr int required_parents(DEVariant v) {
        return 1 + num_difference_vectors(v) * 2;
    }
    ...
    void apply(this auto& self, Population& pop,
               const int* parent_indices, int num_parents,
...
private:
    ...
    double mutate(this auto& self, Population& pop,
                  const int* parent_indices,
                  int gene_idx, DEMutationType mut_type,
                  int best_idx, int n_diff) {
        switch (mut_type) {
