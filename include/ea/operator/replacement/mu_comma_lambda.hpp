#pragma once
/// @file mu_comma_lambda.hpp
/// @brief (μ,λ) Replacement — non-elitist replacement from offspring only.
///
/// Only offspring compete for survival. The best μ offspring form the next generation.
/// Parents are discarded regardless of quality. This allows the population to escape
/// local optima but risks losing good solutions.
///
/// Reference: jMetal
...
/// (μ,λ) Replacement — keep best μ from offspring only.
...
struct MuCommaLambdaReplacement {
    ...
    std::vector<int> replace(Population& pop, const std::vector<int>& offspring_indices, int mu) {
        ...
        // Compute ranks and crowding distances for offspring
        auto fronts = fast_non_dominated_sort_subset(pop, candidates);
        ...
        for (const auto& front : fronts) {
            if (static_cast<int>(selected.size()) + static_cast<int>(front.size()) <= mu) {
                ...
                // Select from this front by crowding distance
        ...
        // Compact selected into positions [0, mu)
