...
struct Front {
    ...
    explicit Front(int max_points, int n_obj) {
        ...
        for (auto& p : points) p.objectives.resize(n_obj);
    }
};

inline bool beats(double x, double y) { return x > y; }
inline double worse(double x, double y) { return x > y ? y : x; }
...
inline bool dominates1way(const Point& p, const Point& q, int k) {
    for (int i = k; i >= 0; --i)
        if (beats(q.objectives[i], p.objectives[i])) return false;
    return true;
}

inline double inclusive_hv(const Point& p, int n_obj) {
    double volume = 1.0;
    for (int i = 0; i < n_obj; ++i) volume *= p.objectives[i];
    return volume;
}
...
inline double inclusive_hv4(const Point& p, const Point& q, const Point& r, const Point& s, int n_obj) {
    ...
    for (int i = 0; i < n_obj; ++i) {
        double po = p.objectives[i], qo = q.objectives[i], ro = r.objectives[i], so = s.objectives[i];
        vp *= po; vq *= qo; vr *= ro; vs *= so;
        if (beats(po, qo)) {
            if (beats(qo, ro)) {
                if (beats(ro, so)) {
                    vpq *= qo; vpr *= ro; vps *= so; vqr *= ro; vqs *= so; vrs *= so;
                    vpqr *= ro; vpqs *= so; vprs *= so; vqrs *= so; vpqrs *= so;
                } else {
                    double z1 = worse(qo, so);
                    vpq *= qo; vpr *= ro; vps *= worse(po, so); vqr *= ro; vqs *= z1; vrs *= ro;
                    vpqr *= ro; vpqs *= z1; vprs *= ro; vqrs *= ro; vpqrs *= ro;
                }
            } else if (beats(qo, so)) {
                vpq *= qo; vpr *= worse(po, ro); vps *= so; vqr *= qo; vqs *= so; vrs *= so;
                vpqr *= qo; vpqs *= so; vprs *= so; vqrs *= so; vpqrs *= so;
            } else {
                double z1 = worse(po, ro);
                vpq *= qo; vpr *= z1; vps *= worse(po, so); vqr *= qo; vqs *= qo;
                vrs *= worse(ro, so);
                vpqr *= qo; vpqs *= qo; vprs *= worse(z1, so); vqrs *= qo; vpqrs *= qo;
            }
        } else if (beats(qo, ro)) {
            if (beats(po, so)) {
                double z1 = worse(po, ro), z2 = worse(ro, so);
                vpq *= po; vpr *= z1; vps *= so; vqr *= ro; vqs *= so; vrs *= z2;
                vpqr *= z1; vpqs *= so; vprs *= z2; vqrs *= z2; vpqrs *= z2;
            } else {
                double z1 = worse(po, ro), z2 = worse(ro, so);
                vpq *= po; vpr *= z1; vps *= po; vqr *= ro; vqs *= worse(qo, so); vrs *= z2;
                vpqr *= z1; vpqs *= po; vprs *= z1; vqrs *= z2; vpqrs *= z1;
            }
        } else if (beats(po, so)) {
            vpq *= po; vpr *= po; vps *= so; vqr *= qo; vqs *= so; vrs *= so;
            vpqr *= po; vpqs *= so; vprs *= so; vqrs *= so; vpqrs *= so;
        } else {
            double z1 = worse(qo, so);
            vpq *= po; vpr *= po; vps *= po; vqr *= qo; vqs *= z1; vrs *= worse(ro, so);
            vpqr *= po; vpqs *= po; vprs *= po; vqrs *= z1; vpqrs *= po;
        }
    }
    return vp + vq + vr + vs - vpq - vpr - vps - vqr - vqs - vrs
         + vpqr + vpqs + vprs + vqrs - vpqrs;
}
...
inline void make_dominated_bit(Front& ps, int p, int n_obj, std::vector<Front>& fs, int& fr, int& safe) {
    int l = 0;
    int u = p - 1;
    for (int i = p - 1; i >= 0; --i) {
    ...
    for (int i = 1; i < l; ++i) {
        ...
        while (j < fs[fr].n_points) {
            ...
            if (dom == 0) {
                ...
                while (j < fs[fr].n_points - 1 && dominates1way(fs[fr].points[j], fs[fr].points[fs[fr].n_points - 1], n_obj - 1))
                    fs[fr].n_points--;
                int k = j + 1;
...
inline double hv(Front& ps, int n_obj, std::vector<Front>& fs, int& fr, int& safe) {
    switch (ps.n_points) {
        case 1: return inclusive_hv(ps.points[0], n_obj);
        case 2: return inclusive_hv2(ps.points[0], ps.points[1], n_obj);
        case 3: return inclusive_hv3(ps.points[0], ps.points[1], ps.points[2], n_obj);
        case 4: return inclusive_hv4(ps.points[0], ps.points[1], ps.points[2], ps.points[3], n_obj);
        default: break;
    }
    ...
    for (int i = 0; i < ps.n_points; ++i) sorted_points[i] = ps.points[order[i]];
    for (int i = 0; i < ps.n_points; ++i) ps.points[i] = sorted_points[i];

    if (n_obj == 2) return hv2(ps, ps.n_points, n_obj);
    ...
    if (n_obj == 3 && safe > 0) {
        ...
        double volume = inclusive_hv4(ps.points[0], ps.points[1], ps.points[2], ps.points[3], n_obj);
...
inline double hypervolume_2d(std::vector<std::vector<double>> points,
    ...
    points.erase(std::remove_if(points.begin(), points.end(), [](const auto& p) {
        return std::any_of(p.begin(), p.end(), [](double v) { return v <= 0; });
    }), points.end());
...
inline double hypervolume_wfg(const std::vector<std::vector<double>>& points,
                               int n_obj, const std::vector<double>& ref_point) {
    // Translate points
    ...
    for (const auto& p : points) {
        ...
        for (int i = 0; i < n_obj; ++i) {
            ...
            if (tp[i] < 0) dominated = true;
        }
        if (!dominated) translated.push_back(tp);
    }

    if (translated.empty()) return 0.0;
    if (translated.size() == 1) {
        double vol = 1.0;
        for (double v : translated[0]) vol *= v;
        return vol;
    }
...
inline double hypervolume(const Population& pop,
    ...
    for (int idx : indices) {
        ...
        for (int o = 0; o < n_obj; ++o) p[o] = pop.objective(idx, o);
        points.push_back(p);
    }
...
struct HypervolumeIndicator {
    ...
    double compute(this HypervolumeIndicator& self,
                   const std::vector<std::vector<double>>& front) {
        if (self.ref_point.empty() || front.empty()) return 0.0;
    ...
    double compute(this HypervolumeIndicator& self,
                   ...
                   const std::vector<int>& indices) {
        if (self.ref_point.empty() || indices.empty()) return 0.0;
