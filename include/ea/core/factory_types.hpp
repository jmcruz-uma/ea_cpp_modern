...
template<typename T>
struct CrossoverAdapter : CrossoverBase {
    T op;
    explicit CrossoverAdapter(T t = T{}) : op(std::move(t)) {}
...
template<typename T>
struct MutationAdapter : MutationBase {
    T op;
    explicit MutationAdapter(T t = T{}) : op(std::move(t)) {}
...
struct ProblemAdapter : ProblemBase {
    T prob;
    explicit ProblemAdapter(T t = T{}) : prob(std::move(t)) {}
