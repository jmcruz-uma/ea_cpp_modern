#!/usr/bin/env bash
# =============================================================================
# run_wfg_comparison.sh
# Comparativa: ea_cpp_modern (NSGA-II, C++) vs jMetal 7.4 (Java)
# Problemas: WFG1-9 (k=2, l=4, M=2, dim=6)
#
# Uso:
#   ./scripts/run_wfg_comparison.sh [OUTDIR] [NUMRUNS]
#
# Argumentos opcionales:
#   OUTDIR    directorio de salida    (default: results/wfg_comparison)
...
#   SKIP_JAVA=1 ./scripts/run_wfg_comparison.sh /tmp/wfg_test 3
#
# Ejemplo paper (30 runs):
#   ./scripts/run_wfg_comparison.sh results/wfg_paper 30
# =============================================================================
...
REF_DIR="${JMETAL_ROOT}/resources/referenceFrontsCSV"

OUTDIR="${1:-${MODERN_ROOT}/results/wfg_comparison}"
NUMRUNS="${2:-30}"

CPP_BIN="${MODERN_ROOT}/build/wfg_runner"
...
check_prereqs() {
    ...
    if [[ ! -d "${REF_DIR}" ]]; then
        echo "ERROR: directorio de frentes de referencia no encontrado: ${REF_DIR}"
        missing=1
    fi
...
build_java_benchmark() {
    echo "[3/4] Compilando JMetalWFGBenchmark.java..."
    ...
    javac -cp "${JMETAL_CP}" "${BENCH_DIR}/JMetalWFGBenchmark.java" -d "${OUTDIR}"
    echo "      → ${OUTDIR}/JMetalWFGBenchmark.class"
}
...
echo "============================================================"
echo " Algoritmo : NSGA-II (SBX + PolynomialMutation)"
echo " Problemas : WFG1-9 (k=2, l=4, M=2, dim=6)"
echo " Config    : pop=100, max_evals=25000, SBX(pc=0.9, η=20), PM(η=20, p=1/6)"
echo " Runs      : ${NUMRUNS}"
echo " Salida    : ${OUTDIR}"
...
{
    echo "# Entorno de benchmark NSGA-II WFG"
    echo ""
    echo "## Fecha"
    ...
    echo "crossover    : SBX, pc=0.9, eta_c=20"
    echo "mutation     : PolynomialMutation, eta_m=20, pm=1/6"
    echo "Problemas    : WFG1-9 (k=2, l=4, M=2, dim=6)"
    echo "Runs         : ${NUMRUNS}"
    echo "Seed base    : 42"
...
echo "      → ${CPP_CSV}"
"${CPP_BIN}" "${NUMRUNS}" 42 "${REF_DIR}" > "${CPP_CSV}"
echo "      C++ completado."
...
if [[ "$SKIP_JAVA" == "0" ]]; then
    ...
    java -server -Xms512m -Xmx2g \
         ...
         -cp "${JMETAL_CP}:${OUTDIR}" \
         JMetalWFGBenchmark "${JAVA_CSV}" "${NUMRUNS}" 42 "${REF_DIR}"
...
echo ""
echo "Benchmark WFG completado. Resultados en: ${OUTDIR}"
