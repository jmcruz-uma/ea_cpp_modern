#!/usr/bin/env bash
# =============================================================================
# run_moead_comparison.sh
# Comparativa multiobjetivo: ea_cpp_modern (MOEA/D-DE, C++) vs jMetal 7.4 (Java)
# Problemas: ZDT1, ZDT2, ZDT3, ZDT4
#
# Uso:
#   ./scripts/run_moead_comparison.sh [OUTDIR] [NUMRUNS]
#
# Argumentos opcionales:
#   OUTDIR    directorio de salida    (default: results/moead_comparison)
...
#   SKIP_JAVA=1 ./scripts/run_moead_comparison.sh /tmp/moead_test 1
#
# Ejemplo paper (30 runs):
#   ./scripts/run_moead_comparison.sh results/moead_paper 30
# =============================================================================
...
CPP_BIN="${MODERN_ROOT}/build/moead_runner"
...
build_java_benchmark() {
    echo "[3/4] Compilando JMetalMOEADBenchmark.java..."
    ...
    javac -cp "${JMETAL_CP}" "${BENCH_DIR}/JMetalMOEADBenchmark.java" -d "${OUTDIR}"
    echo "      → ${OUTDIR}/JMetalMOEADBenchmark.class"
}
...
echo "============================================================"
echo " Algoritmo : MOEA/D-DE (Tchebycheff, DE/rand/1/bin)"
echo " Problemas : ZDT1 (d=30), ZDT2 (d=30), ZDT3 (d=30), ZDT4 (d=10)"
echo " Config    : pop=100, max_evals=25000, T=20, delta=0.9, nr=2"
echo "             DE: CR=1.0, F=0.5 | PM pm=1/D eta_m=20"
echo " Runs      : ${NUMRUNS}"
echo " Salida    : ${OUTDIR}"
echo "============================================================"
echo ""
...
{
    echo "# Entorno de benchmark MOEA/D-DE"
    echo ""
    echo "## Fecha"
    ...
    echo "Algoritmo    : MOEA/D-DE"
    echo "Variante DE  : RAND_1_BIN, CR=1.0, F=0.5"
    echo "Agregación   : Tchebycheff"
    echo "Problemas    : ZDT1 (d=30), ZDT2 (d=30), ZDT3 (d=30), ZDT4 (d=10)"
    echo "pop_size     : 100"
...
if [[ "$SKIP_JAVA" == "0" ]]; then
    ...
    java -server -Xms512m -Xmx2g \
        ...
        -cp "${JMETAL_CP}:${OUTDIR}" \
        JMetalMOEADBenchmark "${JAVA_CSV}" "${NUMRUNS}" 42
    END=$(date +%s%N)
