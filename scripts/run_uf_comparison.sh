#!/usr/bin/env bash
# =============================================================================
# run_uf_comparison.sh
# Comparativa: ea_cpp_modern (NSGA-II, C++) vs jMetal 7.4 (Java)
# Problemas: UF1-UF7 (CEC2009 Unconstrained, 2 objetivos, 30 variables)
#
# Uso:
#   ./scripts/run_uf_comparison.sh [OUTDIR] [NUMRUNS]
#
# Argumentos opcionales:
#   OUTDIR    directorio de salida    (default: results/uf_comparison)
...
#   SKIP_JAVA=1 ./scripts/run_uf_comparison.sh /tmp/uf_test 3
#
# Ejemplo paper (30 runs):
#   ./scripts/run_uf_comparison.sh results/uf_paper 30
# =============================================================================
...
OUTDIR="${1:-${MODERN_ROOT}/results/uf_comparison}"
NUMRUNS="${2:-30}"

CPP_BIN="${MODERN_ROOT}/build/uf_runner"
...
build_java_benchmark() {
    echo "[3/4] Compilando JMetalUFBenchmark.java..."
    ...
    javac -cp "${JMETAL_CP}" "${BENCH_DIR}/JMetalUFBenchmark.java" -d "${OUTDIR}"
    echo "      → ${OUTDIR}/JMetalUFBenchmark.class"
}
...
echo " Problemas : UF1-UF7 (CEC2009, 2-obj, 30 vars)"
echo " Config    : pop=100, max_evals=25000, SBX(pc=0.9, η=20), PM(η=20, p=1/30)"
echo " Runs      : ${NUMRUNS}"
echo " Salida    : ${OUTDIR}"
echo "============================================================"
echo ""
...
{
    echo "# Entorno de benchmark NSGA-II UF"
    echo ""
    echo "## Fecha"
    ...
    echo "mutation     : PolynomialMutation, eta_m=20, pm=1/30"
    echo "Problemas    : UF1-UF7 (30 vars cada uno)"
    echo "Runs         : ${NUMRUNS}"
    echo "Seed base    : 42"
...
if [[ "$SKIP_JAVA" == "0" ]]; then
    ...
    java -server -Xms512m -Xmx2g \
         ...
         -cp "${JMETAL_CP}:${OUTDIR}" \
         JMetalUFBenchmark "${JAVA_CSV}" "${NUMRUNS}" 42
...
echo ""
echo "Benchmark UF completado. Resultados en: ${OUTDIR}"
