#!/usr/bin/env bash
# =============================================================================
# run_gde3_comparison.sh
# Comparativa: ea_cpp_modern (GDE3, C++) vs jMetal 7.4 (Java)
# Problemas: ZDT1, ZDT2, ZDT3, ZDT4
#
# Uso:
#   ./scripts/run_gde3_comparison.sh [OUTDIR] [NUMRUNS]
#
# Argumentos opcionales:
#   OUTDIR    directorio de salida    (default: results/gde3_comparison)
...
#   SKIP_JAVA=1 ./scripts/run_gde3_comparison.sh /tmp/gde3_test 3
#
# Ejemplo paper (30 runs):
#   ./scripts/run_gde3_comparison.sh results/gde3_paper 30
# =============================================================================
...
OUTDIR="${1:-${MODERN_ROOT}/results/gde3_comparison}"
NUMRUNS="${2:-30}"

CPP_BIN="${MODERN_ROOT}/build/gde3_runner"
...
build_java_benchmark() {
    echo "[3/4] Compilando JMetalGDE3Benchmark.java..."
    ...
    # GDE3 usa core + algorithm + problem + commons-lang3 (NO jmetal-component)
    JMETAL_CP="${CORE_JAR}:${ALGO_JAR}:${PROB_JAR}:${COMMONS_JAR}"
    export JMETAL_CP

    javac -cp "${JMETAL_CP}" "${BENCH_DIR}/JMetalGDE3Benchmark.java" -d "${OUTDIR}"
    echo "      → ${OUTDIR}/JMetalGDE3Benchmark.class"
}
...
echo "============================================================"
echo " Algoritmo : GDE3 (DE/rand/1/bin, CR=0.5, F=0.5)"
...
{
    echo "# Entorno de benchmark GDE3"
    echo ""
    echo "## Fecha"
...
if [[ "$SKIP_JAVA" == "0" ]]; then
    ...
    java -server -Xms512m -Xmx2g \
         ...
         -cp "${JMETAL_CP}:${OUTDIR}" \
         JMetalGDE3Benchmark "${JAVA_CSV}" "${NUMRUNS}" 42
...
echo ""
echo "Benchmark GDE3 completado. Resultados en: ${OUTDIR}"
