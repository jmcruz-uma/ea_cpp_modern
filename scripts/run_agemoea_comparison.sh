#!/usr/bin/env bash
# =============================================================================
# run_agemoea_comparison.sh
# Comparativa: ea_cpp_modern (AGE-MOEA, C++) vs jMetal 7.4 (Java)
# Problemas: ZDT1, ZDT2, ZDT3, ZDT4
#
# Uso:
#   ./scripts/run_agemoea_comparison.sh [OUTDIR] [NUMRUNS]
#
# Argumentos opcionales:
#   OUTDIR    directorio de salida    (default: results/agemoea_comparison)
...
#   SKIP_JAVA=1 ./scripts/run_agemoea_comparison.sh /tmp/agemoea_test 3
#
# Ejemplo paper (30 runs):
#   ./scripts/run_agemoea_comparison.sh results/agemoea_paper 30
# =============================================================================
...
OUTDIR="${1:-${MODERN_ROOT}/results/agemoea_comparison}"
NUMRUNS="${2:-30}"

CPP_BIN="${MODERN_ROOT}/build/agemoea_runner"
...
build_cpp() {
    echo "[1/4] Compilando ea_cpp_modern (AGE-MOEA runner)..."
    mkdir -p "${MODERN_ROOT}/build"
    ${CXX_BENCH} -std=c++23 -O3 -march=native -DNDEBUG \
        -I "${MODERN_ROOT}/include" \
        "${MODERN_ROOT}/examples/agemoea_runner.cpp" \
        -o "${CPP_BIN}" -lm
    echo "      → ${CPP_BIN}"
}
...
build_java_benchmark() {
    echo "[3/4] Compilando JMetalAGEMOEABenchmark.java..."
    ...
    # AGE-MOEA usa core + algorithm + problem + commons-lang3 (NO jmetal-component)
    JMETAL_CP="${CORE_JAR}:${ALGO_JAR}:${PROB_JAR}:${COMMONS_JAR}"
    export JMETAL_CP

    javac -cp "${JMETAL_CP}" "${BENCH_DIR}/JMetalAGEMOEABenchmark.java" -d "${OUTDIR}"
    echo "      → ${OUTDIR}/JMetalAGEMOEABenchmark.class"
}
...
echo "============================================================"
echo " Algoritmo : AGE-MOEA (SBX cx_prob=0.9 eta=20, PM eta=20)"
echo " Problemas : ZDT1 (d=30), ZDT2 (d=30), ZDT3 (d=30), ZDT4 (d=10)"
echo " Config    : pop=100, max_evals=25000 (Java: max_iter=250)"
echo " Runs      : ${NUMRUNS}"
echo " Salida    : ${OUTDIR}"
echo "============================================================"
echo ""
...
{
    echo "# Entorno de benchmark AGE-MOEA"
    echo ""
    echo "## Fecha"
    ...
    echo "Algoritmo    : AGE-MOEA"
    echo "pop_size     : 100"
    echo "max_evals    : 25000 (C++) / max_iter=250 (Java)"
    echo "cx_prob      : 0.9"
    echo "cx_eta       : 20.0"
    echo "mut_eta      : 20.0"
    echo "Problemas    : ZDT1(d=30), ZDT2(d=30), ZDT3(d=30), ZDT4(d=10)"
...
if [[ "$SKIP_JAVA" == "0" ]]; then
    ...
    java -server -Xms512m -Xmx2g \
         ...
         -cp "${JMETAL_CP}:${OUTDIR}" \
         JMetalAGEMOEABenchmark "${JAVA_CSV}" "${NUMRUNS}" 42
...
echo ""
echo "Benchmark AGE-MOEA completado. Resultados en: ${OUTDIR}"
