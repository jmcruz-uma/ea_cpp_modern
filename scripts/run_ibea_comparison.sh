#!/usr/bin/env bash
# =============================================================================
# run_ibea_comparison.sh
# Comparativa multiobjetivo: ea_cpp_modern (IBEA, C++) vs jMetal 7.4 (Java)
# Problemas: ZDT1, ZDT2, ZDT3, ZDT4
#
# Uso:
#   ./scripts/run_ibea_comparison.sh [OUTDIR] [NUMRUNS]
#
# Argumentos opcionales:
#   OUTDIR    directorio de salida    (default: results/ibea_comparison)
...
#   SKIP_JAVA=1 ./scripts/run_ibea_comparison.sh /tmp/ibea_test 1
#
# Ejemplo paper (30 runs):
#   ./scripts/run_ibea_comparison.sh results/ibea_paper 30
# =============================================================================
...
OUTDIR="${1:-${MODERN_ROOT}/results/ibea_comparison}"
NUMRUNS="${2:-30}"

CPP_BIN="${MODERN_ROOT}/build/ibea_runner"
...
build_java_benchmark() {
    echo "[3/4] Compilando JMetalIBEABenchmark.java..."

    # Individual module jars (same classpath protocol as other benchmarks)
    ...
    javac -cp "${JMETAL_CP}" "${BENCH_DIR}/JMetalIBEABenchmark.java" -d "${OUTDIR}"
    echo "      → ${OUTDIR}/JMetalIBEABenchmark.class"
}
...
echo "============================================================"
echo " Algoritmo : IBEA (HVI indicator, rho=2.0, kappa=0.05)"
...
echo "             Selección: BinaryTournament (FitnessComparator)"
echo " Runs      : ${NUMRUNS}"
echo " Salida    : ${OUTDIR}"
echo "============================================================"
echo ""
...
{
    echo "# Entorno de benchmark IBEA"
    echo ""
    echo "## Fecha"
    ...
    echo "Algoritmo    : IBEA"
    echo "Indicator    : HVI (hypervolume), rho=2.0, kappa=0.05"
    echo "Problemas    : ZDT1 (d=30), ZDT2 (d=30), ZDT3 (d=30), ZDT4 (d=10)"
    echo "pop_size     : 100"
...
if [[ "$SKIP_JAVA" == "0" ]]; then
    ...
    java -server -Xms512m -Xmx2g \
        ...
        -cp "${JMETAL_CP}:${OUTDIR}" \
        JMetalIBEABenchmark "${JAVA_CSV}" "${NUMRUNS}" 42
    END=$(date +%s%N)
