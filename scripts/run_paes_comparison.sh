#!/usr/bin/env bash
# =============================================================================
# run_paes_comparison.sh
# Comparativa: ea_cpp_modern (PAES, C++) vs jMetal 7.4 (Java)
# Problemas: ZDT1, ZDT2, ZDT3, ZDT4
#
# Uso:
#   ./scripts/run_paes_comparison.sh [OUTDIR] [NUMRUNS]
#
# Argumentos opcionales:
#   OUTDIR    directorio de salida    (default: results/paes_comparison)
...
#   SKIP_JAVA=1 ./scripts/run_paes_comparison.sh /tmp/paes_test 3
#
# Ejemplo paper (30 runs):
#   ./scripts/run_paes_comparison.sh results/paes_paper 30
# =============================================================================
...
OUTDIR="${1:-${MODERN_ROOT}/results/paes_comparison}"
NUMRUNS="${2:-30}"

CPP_BIN="${MODERN_ROOT}/build/paes_runner"
...
build_java_benchmark() {
    echo "[3/4] Compilando JMetalPAESBenchmark.java..."
    ...
    javac -cp "${JMETAL_CP}" "${BENCH_DIR}/JMetalPAESBenchmark.java" -d "${OUTDIR}"
    echo "      → ${OUTDIR}/JMetalPAESBenchmark.class"
}
...
echo "============================================================"
echo " Algoritmo : PAES (GenericBoundedArchive + GridDensityEstimator)"
echo " Operadores: PolynomialMutation eta=20 prob=1/dim"
echo " Problemas : ZDT1 (d=30), ZDT2 (d=30), ZDT3 (d=30), ZDT4 (d=10)"
echo " Config    : max_evals=25000, archive=100, bisections=5"
echo " Runs      : ${NUMRUNS}"
echo " Salida    : ${OUTDIR}"
echo "============================================================"
echo ""
...
{
    echo "# Entorno de benchmark PAES"
    echo ""
    echo "## Fecha"
    ...
    echo "archive_size : 100  (GenericBoundedArchive + GridDensityEstimator)"
    echo "bisections   : 5"
    echo "mut_eta      : 20.0"
    echo "mut_prob     : 1/dim"
    echo "Problemas    : ZDT1(d=30), ZDT2(d=30), ZDT3(d=30), ZDT4(d=10)"
...
if [[ "$SKIP_JAVA" == "0" ]]; then
    ...
    java -server -Xms512m -Xmx2g \
         ...
         -cp "${JMETAL_CP}:${OUTDIR}" \
         JMetalPAESBenchmark "${JAVA_CSV}" "${NUMRUNS}" 42
...
echo ""
echo "Benchmark PAES completado. Resultados en: ${OUTDIR}"
