#!/usr/bin/env bash
# =============================================================================
# run_lz09_comparison.sh
# Comparativa: ea_cpp_modern (C++) vs jMetal 7.4 (Java)
# Problemas: LZ09F1-F9 (Li & Zhang 2009)
#   - LZ09F1-F5, F7-F9: NSGA-II (2 obj)
#   - LZ09F6:           NSGA-III (3 obj, divisions=12)
#
# Uso:
#   ./scripts/run_lz09_comparison.sh [OUTDIR] [NUMRUNS]
#
# Argumentos opcionales:
#   OUTDIR    directorio de salida    (default: results/lz09_comparison)
...
#   SKIP_JAVA=1 ./scripts/run_lz09_comparison.sh /tmp/lz09_test 3
#
# Ejemplo paper (30 runs):
#   ./scripts/run_lz09_comparison.sh results/lz09_paper 30
# =============================================================================
...
OUTDIR="${1:-${MODERN_ROOT}/results/lz09_comparison}"
NUMRUNS="${2:-30}"

CPP_BIN="${MODERN_ROOT}/build/lz09_runner"
...
check_prereqs() {
    ...
    if ! command -v ${CXX_BENCH} &>/dev/null; then
        echo "ERROR: ${CXX_BENCH} no encontrado."
        missing=1
    fi
    if [[ "$SKIP_JAVA" == "0" ]]; then
...
build_java_benchmark() {
    echo "[3/4] Compilando JMetalLZ09Benchmark.java..."
    ...
    javac -cp "${JMETAL_CP}" "${BENCH_DIR}/JMetalLZ09Benchmark.java" -d "${OUTDIR}"
    echo "      → ${OUTDIR}/JMetalLZ09Benchmark.class"
}
...
echo "============================================================"
echo " Problemas : LZ09F1-F9 (Li & Zhang 2009)"
echo " F1-F5,F7-F9: NSGA-II, 2-obj, pop=100, max_evals=25000"
echo " F6        : NSGA-III, 3-obj, divisions=12, pop=92"
echo " Operadores: SBX(pc=0.9, η=20), PM(η=20, p=1/dim)"
echo " Runs      : ${NUMRUNS}"
echo " Salida    : ${OUTDIR}"
echo "============================================================"
echo ""
...
{
    echo "# Entorno de benchmark LZ09"
    echo ""
    echo "## Fecha"
    ...
    echo "Problemas    : LZ09F1-F9"
    echo "NSGA-II      : pop=100, max_evals=25000, SBX(pc=0.9, η=20), PM(η=20, p=1/dim)"
    echo "NSGA-III(F6) : divisions=12, pop=92, max_evals≈25000"
    echo "Runs         : ${NUMRUNS}"
    echo "Seed base    : 42"
...
if [[ "$SKIP_JAVA" == "0" ]]; then
    ...
    java -server -Xms512m -Xmx2g \
         ...
         -cp "${JMETAL_CP}:${OUTDIR}" \
         JMetalLZ09Benchmark "${JAVA_CSV}" "${NUMRUNS}" 42
...
try:
    ...
    for p, d in sorted(data.items()):
        ...
        print(f'  {p:<10}: median time={tm:8.1f}ms  median IGD={im:.6f}')
...
echo ""
echo "Benchmark LZ09 completado. Resultados en: ${OUTDIR}"
