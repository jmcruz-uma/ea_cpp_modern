#!/usr/bin/env bash
# =============================================================================
# run_mocell_comparison.sh
# Comparativa: ea_cpp_modern (MOCell, C++) vs jMetal 7.4 (Java)
# Problemas: ZDT1, ZDT2, ZDT3, ZDT4
#
# Uso:
#   ./scripts/run_mocell_comparison.sh [OUTDIR] [NUMRUNS]
#
# Argumentos opcionales:
#   OUTDIR    directorio de salida    (default: results/mocell_comparison)
...
#   SKIP_JAVA=1 ./scripts/run_mocell_comparison.sh /tmp/mocell_test 3
#
# Ejemplo paper (30 runs):
#   ./scripts/run_mocell_comparison.sh results/mocell_paper 30
# =============================================================================
...
OUTDIR="${1:-${MODERN_ROOT}/results/mocell_comparison}"
NUMRUNS="${2:-30}"

CPP_BIN="${MODERN_ROOT}/build/mocell_runner"
...
build_java_benchmark() {
    echo "[3/4] Compilando JMetalMOCellBenchmark.java..."
    ...
    javac -cp "${JMETAL_CP}" "${BENCH_DIR}/JMetalMOCellBenchmark.java" -d "${OUTDIR}"
    echo "      → ${OUTDIR}/JMetalMOCellBenchmark.class"
}
...
echo "============================================================"
echo " Algoritmo : MOCell (C9 neighborhood, CrowdingDistanceArchive)"
echo " Operadores: SBX cx_prob=0.9 eta=20, PM eta=20"
...
{
    echo "# Entorno de benchmark MOCell"
    echo ""
    echo "## Fecha"
    ...
    echo "Algoritmo    : MOCell"
    echo "pop_size     : 100  (grid 10×10)"
    echo "archive_size : 100  (CrowdingDistanceArchive)"
    echo "max_evals    : 25000 (offspring; initial 100 not counted)"
    echo "cx_prob      : 0.9"
    echo "cx_eta       : 20.0"
    echo "mut_eta      : 20.0"
    echo "Problemas    : ZDT1(d=30), ZDT2(d=30), ZDT3(d=30), ZDT4(d=10)"
...
if [[ "$SKIP_JAVA" == "0" ]]; then
    ...
    java -server -Xms512m -Xmx2g \
         ...
         -cp "${JMETAL_CP}:${OUTDIR}" \
         JMetalMOCellBenchmark "${JAVA_CSV}" "${NUMRUNS}" 42
...
echo ""
echo "Benchmark MOCell completado. Resultados en: ${OUTDIR}"
